...
router.post('/analyze-context', async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Get spreadsheet data from multiple sources
    ...
    if (req.file) {
    ...
    } else if (providedSpreadsheetData) {
      // Handle spreadsheet data provided in request body (for testing/API calls)
      console.log(`[${requestId}] Using spreadsheet data provided in request body`);
      spreadsheetData = providedSpreadsheetData;
      ...
      return res.status(400).json({
        ...
        error: {
          ...
          message: 'No spreadsheet data available. Please upload a file, provide spreadsheet data in the request body, or provide a valid spreadsheet ID.',
          timestamp: new Date().toISOString()
        },
        requestId,
        processingTime: Date.now() - startTime
